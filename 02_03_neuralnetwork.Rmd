...
terminator_evals50 = trm("evals", n_evals = 5)

# 4) Auto‐tuners with the pipe in front

# 1. Sæt ID på din base learner
base_nnet = lrn("regr.nnet", id = "nnet")

# 2. Lav GraphLearner
gl_nnet = make_graph_learner(base_nnet)

# 3. Definér search space med navnerum
search_space_nnet = ps(
  nnet.size  = p_int(lower = 1, upper = 20),
  nnet.decay = p_dbl(lower = 1e-5, upper = 1e-1)
)

# 4. AutoTuner med korrekt search space
at_nnet = AutoTuner$new(
  learner      = gl_nnet,
...
learners = list(at_nnet, gl_featless)
